...
def test_ollama_integration():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if available_compatible:
                print(f"✅ Compatible models found: {', '.join(available_compatible[:3])}")
    ...
    try:
        result = subprocess.run(['ollmcp', '--help'], 
                              capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
    ...
    try:
        ...
        if result.returncode == 0 and "✅ API spec fetching and validation test completed successfully" in result.stdout:
    ...
    if config_path.exists():
        ...
        print("   Copy from template: cp equinix-mcp-config.template.json equinix-mcp-config.json")
...
'''
    ...
    try:
        ...
        proc = subprocess.Popen([
            'ollmcp', '--servers-json', 'equinix-mcp-config.json', '--model', recommended_model, '--help'
    ...
    print(f"   ollmcp --servers-json equinix-mcp-config.json --model {recommended_model}")
