...
class AuthManager:
    ...
    async def _get_metal_auth_header(self) -> Dict[str, str]:
        ...
        if not self.metal_token:
            logger.error("EQUINIX_METAL_TOKEN environment variable is required for Metal API")
            raise ValueError(
    ...
    async def _get_client_credentials_auth_header(self) -> Dict[str, str]:
        ...
        if not self.client_id or not self.client_secret:
            logger.error("EQUINIX_CLIENT_ID and EQUINIX_CLIENT_SECRET environment variables are required")
            raise ValueError(
    ...
    async def _get_access_token(self) -> str:
        ...
        logger.debug(f"Token request data: {{'grant_type': 'client_credentials', 'client_id': '{self.client_id}', 'client_secret': '[REDACTED]'}}")
