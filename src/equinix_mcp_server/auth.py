...
class AuthManager:
    ...
    async def _get_metal_auth_header(self) -> Dict[str, str]:
        ...
        if not self.metal_token:
            raise ValueError("EQUINIX_METAL_TOKEN environment variable is required for Metal API")
    ...
    async def _get_client_credentials_auth_header(self) -> Dict[str, str]:
        ...
        if not self.client_id or not self.client_secret:
            raise ValueError("EQUINIX_CLIENT_ID and EQUINIX_CLIENT_SECRET environment variables are required")
