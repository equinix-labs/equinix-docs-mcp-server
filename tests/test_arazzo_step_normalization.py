...
class DummyStepExecutor:
    ...
    def determine_next_action(self, step_def, success, state):  # pragma: no cover
        return {
            "type": type("A", (), {"END": "END"}).END if False else {"type": "END"}
        }  # not used
...
async def test_simplified_step_normalization(tmp_path, monkeypatch):
    ...
    class DummyRunner:
            ...
            for workflow_key, workflow in workflows.items():
                if "steps" in workflow:
                    for step in workflow["steps"]:
                        ...
                        if "operation" in step and not any(
                            k in step for k in ("operationId", "operationPath")
                        ):
        ...
        def execute_workflow(self, workflow_id, inputs):
            ...
            assert all(
                any(k in s for k in ("operationId", "operationPath")) for s in steps
            )
            return {
    ...
    status = getattr(
        res, "status", res.get("status") if isinstance(res, dict) else None
    )
