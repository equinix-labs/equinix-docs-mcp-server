...
async def demonstrate_network_edge_tools():
    ...
    device_listing_ops = [
        op
        for op in network_edge_ops
        if "get" in op["method"].lower()
        and ("ves" in op["path"] or "device" in op["path"].lower())
    ]
    ...
    print("\n🔧 What should happen:")
    print(
        "   1. MCP client identifies relevant tool (e.g., 'network-edge_network-edge_getVirtualDevicesUsingGET_1')"
    )
    print("   2. Calls the tool with appropriate parameters")
    print(
        "   3. Gets authentication error (without credentials) or device list (with credentials)"
    )
